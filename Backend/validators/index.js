...
const rules = {
  ...
  addResident: [
    ...
    body("email").isEmail().withMessage("Valid email is required"),
    body("phone").notEmpty().withMessage("Phone is required"),
    body("role").isIn(["HEAD", "MEMBER"]).withMessage("Role must be HEAD or MEMBER"),
  ],
  createFlat: [
    ...
    body("ownershipType").isIn(["OWNER", "TENANT"]).withMessage("Ownership type must be OWNER or TENANT"),
  ],
  createCategory: [
    body("name").notEmpty().withMessage("Category name is required"),
    body("type").isIn(["MARKETPLACE", "SERVICE"]).withMessage("Type must be MARKETPLACE or SERVICE"),
  ],
  createListing: [
    body("title").notEmpty().withMessage("Title is required"),
    body("description").notEmpty().withMessage("Description is required"),
    body("price").isFloat({ min: 0 }).withMessage("Price must be a non-negative number"),
    body("condition").isIn(["NEW", "LIKE_NEW", "GOOD", "FAIR", "POOR"]).withMessage("Invalid condition value"),
  ],
  createComplaint: [
  ...
  updateComplaintStatus: [
    ...
    // assignedTo is a ServiceProvider._id — just validate it's a valid MongoId if provided
    // Do NOT restrict to User IDs
    body("assignedTo")
      .optional({ nullable: true, checkFalsy: true })
      .isMongoId()
      .withMessage("assignedTo must be a valid ID"),
  ],
  addService: [
  ...
  addReview: [
    ...
    body("targetType").isIn(["SERVICE", "LISTING", "SELLER"]).withMessage("Invalid target type"),
